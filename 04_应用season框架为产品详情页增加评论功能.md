...
>此项目由于时间原因，只完成初步season框架搭建和简单评论显示、发表，后期会进一步调试增加多商品ID评论和删除、修改等功能。
...
**4.创建实体类**
```java
package com.vo;import com.season.core.db.Pojo;
...
* 数据层 
*/
@Repositorypublic class CommentDao {    
//评论数据的保存    
...
*/
@ControllerKey
("product")public class CommentController extends Controller {    
...
**7.数据源配置文件application.properties**
```
server.context-path=/season
season.datasources[0].id=master
season.datasources[0].url=jdbc:mysql://localhost:3306/produc_text?useUnicode=true&characterEncoding=UTF-8&autoReconnect=true&failOverReadOnly=false&zeroDateTimeBehavior=convertToNull
...
import com.vo.Comment;import java.util.List;
/** 
* Created by mwkang on 2016/12/19. 
* 服务层 
*/
                    ...
                    success: function (data) {
                        ...
                        $.each(data.comments, function (commentIndex, comment) {
                            commentsText += "<li>" +
                                    ...
                                    "<div style='background-color: #fff3f3'>" + "评论时间：" + new Date(comment.createDate).getFullYear()+"."+new Date(comment.createDate).getUTCMonth()+"."+new Date(comment.createDate).getDate()+"&nbsp&nbsp&nbsp&nbsp"+ new Date(comment.createDate).getHours()+":"+new Date(comment.createDate).getMinutes() +
                                    "</li>";
                        });
...
**11.http://localhost:8080/season/index.html 运行结果显示如下：
- 首页
